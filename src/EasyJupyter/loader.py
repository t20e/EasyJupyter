...
class EasyJupyterLoader(importlib.abc.Loader):
    ...
    def exec_module(self, module):
        ...
        try:
            # Compile the code object with the file path before executing. This helps us trace the path to the file with the warnings
            compiled_code = compile(code, module.__file__, "exec")
            exec(compiled_code, module.__dict__)
        except Exception as e:
            self.report_notebook_error(e)

    def get_code(self, force_sync=False):
        ...
        if shadow_path.exists():
            ...
            # Cache is fresh and we aren't forcing a sync, read and return it
            if shadow_mtime > nb_mtime and not force_sync:
    ...
    def transform_notebook(self):
        ...
        for cell_idx, cell in enumerate(nb_cells):
            ...
            # Safety fetch the cell ID to support older version fo Jupyter notebooks that don't have the ID field
            cell_id = cell.get("id", "no_id")
            exec_code_extract.append(f"# Cell {cell_idx} | ID: {cell_id}")
        ...
        if warnings:
            nb_name = os.path.relpath(self.path, PROJECT_ROOT)

            formatted_warnings = "\\n".join([f"• {w}" for w in warnings]).replace(
                '"', '\\"'
            )
            ...
            warning_imports = [
                ...
                "from rich.panel import Panel",
                "__ej_console = Console()",
                f'__ej_console.print(Panel("{formatted_warnings}", title="[bold yellow]⚠️ EasyJupyter Syntax Warnings: {nb_name}[/bold yellow]", border_style="yellow"))',
                "del __ej_console",  # Clean up the namespace so these variables don't leak into the user's project
            ]

            for w in warnings:
                # Also write to watcher.log file
                print(f"[WARNING] {nb_name} - {w}")
